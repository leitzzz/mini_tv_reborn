...
def fetch_weather_data(city_coords=None):
    ...
    response = requests.get(
        f"http://api.open-meteo.com/v1/forecast?latitude={latitude}&longitude={longitude}&current=temperature_180m%2Cwind_speed_10m%2Crelative_humidity_2m%2Cprecipitation_probability%2Crain", timeout=5, headers=headers)
    data = response.json()

    return {
        "temperature": f"{data["current"]["temperature_180m"]} {data["current_units"]["temperature_180m"]}",
